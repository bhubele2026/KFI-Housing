...
describe("Dashboard Payroll reconciliation counters", () => {
  // Backend tests already cover the chargeSource projection; these tests
  // pin down the three counter tiles on the dashboard so a refactor of
  // the Payroll reconciliation card can't silently swap, drop, or
  // miscount any of: auto-reconciled, manual, or total Active occupants.
  let container: HTMLDivElement;
  let root: Root | null = null;
  ...
  function getCount(testId: string): string | null {
    const el = container.querySelector(`[data-testid="${testId}"]`);
    return el ? (el.textContent ?? "").trim() : null;
  }

  it("hides the entire payroll reconciliation card when there are no Active occupants", async () => {
    // Card is gated on activeOccupants.length > 0 — Former-only and empty
    // datasets must not show a "0 / 0 / 0" tile that would mislead the
    // operator into thinking nothing has been reconciled.
    mockData.properties = [
      { id: "p1", name: "Lakeside", customerId: "c1", monthlyRent: 100, totalBeds: 1, ratings: {}, paymentNotes: "", notes: "" },
    ];
    mockData.occupants = [
      { id: "o-former", propertyId: "p1", status: "Former", chargeSource: "payroll", moveInDate: "2024-01-01" },
    ];

    await render();

    expect(container.querySelector('[data-testid="card-payroll-reconciliation"]')).toBeNull();
    expect(container.querySelector('[data-testid="text-payroll-auto-reconciled-count"]')).toBeNull();
  });

  it("counts only Active occupants and splits payroll vs manual sources", async () => {
    mockData.properties = [
      { id: "p1", name: "Lakeside", customerId: "c1", monthlyRent: 100, totalBeds: 1, ratings: {}, paymentNotes: "", notes: "" },
    ];
    mockData.occupants = [
      // 2 auto-reconciled (payroll), 3 manual/other → total 5 Active.
      { id: "o1", propertyId: "p1", status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
      { id: "o2", propertyId: "p1", status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
      { id: "o3", propertyId: "p1", status: "Active", chargeSource: "manual",  moveInDate: "2024-01-01" },
      { id: "o4", propertyId: "p1", status: "Active",                          moveInDate: "2024-01-01" },
      { id: "o5", propertyId: "p1", status: "Active", chargeSource: null,      moveInDate: "2024-01-01" },
      // Former occupants must NOT be counted in any of the three tiles.
      { id: "o6", propertyId: "p1", status: "Former", chargeSource: "payroll", moveInDate: "2023-01-01" },
      { id: "o7", propertyId: "p1", status: "Former", chargeSource: "manual",  moveInDate: "2023-01-01" },
    ];

    await render();

    expect(container.querySelector('[data-testid="card-payroll-reconciliation"]')).not.toBeNull();
    expect(getCount("text-payroll-auto-reconciled-count")).toBe("2");
    expect(getCount("text-payroll-manual-count")).toBe("3");
    expect(getCount("text-payroll-total-count")).toBe("5");
  });

  it("respects the active customer scope when computing each counter", async () => {
    mockData.properties = [
      { id: "p1", name: "Lakeside", customerId: "c1", monthlyRent: 100, totalBeds: 1, ratings: {}, paymentNotes: "", notes: "" },
      { id: "p2", name: "Hillside", customerId: "c2", monthlyRent: 100, totalBeds: 1, ratings: {}, paymentNotes: "", notes: "" },
    ];
    mockData.occupants = [
      // c1 → 1 payroll + 2 manual = 3 active
      { id: "o-c1-a", propertyId: "p1", status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
      { id: "o-c1-b", propertyId: "p1", status: "Active", chargeSource: "manual",  moveInDate: "2024-01-01" },
      { id: "o-c1-c", propertyId: "p1", status: "Active",                          moveInDate: "2024-01-01" },
      // c2 → 2 payroll + 0 manual = 2 active
      { id: "o-c2-a", propertyId: "p2", status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
      { id: "o-c2-b", propertyId: "p2", status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
      // Unassigned occupant → never counted regardless of scope.
      { id: "o-x",    propertyId: null, status: "Active", chargeSource: "payroll", moveInDate: "2024-01-01" },
    ];

    await render();

    // All-customers default rolls up both customers' Active occupants.
    expect(getCount("text-payroll-auto-reconciled-count")).toBe("3");
    expect(getCount("text-payroll-manual-count")).toBe("2");
    expect(getCount("text-payroll-total-count")).toBe("5");

    // Scope to c1 — counts collapse to that customer only.
    const handler = selectHandlers.get(FILTER_TESTID);
    if (!handler) throw new Error("filter handler missing");
    await act(async () => {
      handler.onValueChange("c1");
    ...
    expect(getCount("text-payroll-auto-reconciled-count")).toBe("1");
    expect(getCount("text-payroll-manual-count")).toBe("2");
    expect(getCount("text-payroll-total-count")).toBe("3");

    // Scope to c2 — counts flip to c2's pure-payroll roster.
    await act(async () => {
      handler.onValueChange("c2");
    });

    expect(getCount("text-payroll-auto-reconciled-count")).toBe("2");
    expect(getCount("text-payroll-manual-count")).toBe("0");
    expect(getCount("text-payroll-total-count")).toBe("2");
  });
});
