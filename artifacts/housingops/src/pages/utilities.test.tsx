...
describe("Utilities customer filter URL persistence", () => {
  let container: HTMLDivElement;
  let root: Root | null = null;
  ...
  it("selecting a customer adds ?customer=<id> to the URL", async () => {
    ...
    expect(window.location.pathname).toBe("/utilities");
  ...
  it("switching back to All Customers removes the ?customer param", async () => {
    await renderAt("/utilities?customer=c1");
  ...
  it("loading /utilities?customer=<id> pre-selects that customer", async () => {
    await renderAt("/utilities?customer=c2");
  ...
  it("falls back to All Customers when the URL carries an unknown customer id", async () => {
    await renderAt("/utilities?customer=does-not-exist");
  ...
  it("strips an unknown customer id but preserves other query params", async () => {
    await renderAt("/utilities?customer=does-not-exist&other=keep");
  ...
  it("preserves other unrelated query params when toggling the filter", async () => {
    await renderAt("/utilities?other=keep");
