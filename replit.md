...
- **artifacts/api-server** — Express API server. Owns CRUD endpoints for HousingOps (`/api/customers`, `/api/properties`, `/api/leases`, `/api/beds`, `/api/occupants`, `/api/utilities`, `/api/insurance-certificates`) plus `/api/healthz`. `DELETE /api/customers/:id` returns 409 if any property still references the customer. Seeds Postgres with sample housing data on first start (`src/lib/seed.ts`) — 3 customers each owning 1–2 of the 5 sample properties. On every startup it also runs `seedHousingDeductions()` (`src/lib/seed-housing-deductions.ts`), which idempotently applies weekly deductions from the `EE_Housing_Deduciton_by_Customer` payroll export (rows are embedded in the file as `HOUSING_DEDUCTION_ROWS`). It matches existing occupants by `employeeId == personId`, then `(name, company)` case-insensitive, then unique-name-only as a last resort; it never inserts new occupants — unmatched rows are warn-logged for reconciliation.
  - **Digest recipients (Task #410)**: REST surface at `/api/digest-recipients` (GET/POST + DELETE by id) lets admins manage which emails receive the weekly lease-expiry digest in-app. Recipients are stored in the `digest_recipients` DB table. The scheduler merges DB recipients with the legacy `LEASE_DIGEST_RECIPIENTS` env var on each tick, so changes take effect without a redeploy. Frontend settings page at `/settings`.
  - **Insurance certificates (Tasks #333, #334, #397)**: REST surface at `/api/insurance-certificates` (GET/POST + PATCH/DELETE by id) for the documented manual intake path, plus the property-detail Insurance tab and dashboard expiry alerts (≤30 days) UI on top. Per-property PDF seeders share `src/lib/seed-insurance-certificates.ts` (`applyInsuranceCertificates(tx, specs)`) which dedupes by `(propertyId, policyNumber)`. The chateau-knoll seeder loads its real cert; attached-leases / patriot-baraboo / park-place wire empty `CERTIFICATES` arrays today (no source PDFs attached for AutoZone Jeannette 6481/6454, Ridge Motor Inn, Patriot Baraboo, or Park Place) — operators add a single spec entry when a cert PDF is later attached, otherwise certs received by email get POSTed directly. **PDF upload (Task #397)**: the Add Certificate dialog now has a file-picker that uploads the ACORD 25 PDF to GCS object storage via presigned URL (`POST /api/storage/uploads/request-url` → PUT to GCS) and auto-populates `documentUrl` with the serving path (`/api/storage/objects/…`). Existing cert rows without a document show an inline "Upload" button. The list view's "View PDF" link resolves uploaded paths, legacy `/api/attached-assets/` filenames, and external URLs.
  - **Object storage (Task #397)**: Replit App Storage (GCS) provisioned. Server: `src/lib/objectStorage.ts`, `src/routes/storage.ts`. Client: `lib/object-storage-web` (`useUpload` hook, `ObjectUploader` component). OpenAPI spec includes `UploadUrlRequest`/`UploadUrlResponse`/`ErrorEnvelope` schemas.
